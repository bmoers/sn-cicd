...
module.exports = function ({
    ...
    const getUpdateSetFiles = (updateSetSysId, pageCallBack) => {
        return client.get({
            url: `api/swre/v1/cicd/updateset_files/${updateSetSysId}`,
            qs: {
    ...
    const getAllTestInSuites = (pageCallBack) => {
        return client.get({
            url: `api/swre/v1/cicd/test_in_suites`
        }, pageCallBack);
    };
    ...
    const setUpdateSetStatus = (updateSetSysId, state) => {
        return client.patch({
            url: `api/swre/v1/cicd/updateset_status/${updateSetSysId}`
        }, {
            state: state
        });
    };
    ...
    const deployUpdateSet = (updateSetSysId, targetHostName, targetAuth) => {
        ...
        return promiseFor(function (nextOptions) {
            ...
            method: 'POST',
            url: hostName.concat('/api/swre/v1/cicd/deploy'),
            body: {
    ...
    const exportUpdateSet = (updateSetSysId) => {
        return client.get({
            url: `api/swre/v1/cicd/export_updateset/${updateSetSysId}`,
            rawResponse: true
        }).then((response) => ({

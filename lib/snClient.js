    ...
    const tableApiFunctions = () => {
        ...
        const getUpdateSetDetails = (updateSetSysId) => {
            ...
            return client.get({
                ...
                var us = result[0];
                const updateSet = {
                    sys_id: trim(us.sys_id.value),
                    appName: trim(us['application.name'].display_value),
                    scopeName: trim(us['application.scope'].display_value),
                    scopeId: trim(us['application.scope'].value)
                };

                ['name', 'description', 'state', 'remote_sys_id', 'sys_created_by', 'sys_created_on', 'sys_updated_by', 'sys_updated_on'].forEach((field) => {
                    updateSet[field] = trim(us[field].value);
                });
                return updateSet;
         ...
         * @returns {Promise}
         */
        const deployUpdateSet = (updateSetSysId, targetHostName, targetToken) => {
            ...
            return promiseFor(function (nextOptions) {
                ...
                return new SnRestClient({
                    ...
                    if (response.statusCode === 202) { // job created, come back to url
                        ...
                        options.oauthToken = targetToken || options.oauthToken;
                        return options;
                    }

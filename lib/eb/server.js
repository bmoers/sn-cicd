...
*/
    ...
    workerSpace.on('connection', function (socket) {
        ...
        socket.on('done', async (job, callback = () => { }) => {
            ...
            console.log(`${chalk.green(`[${socket.id}]`)} : [worker.done] Job: ${job._id} Type: ${job.type}`);
    ...
    busSpace.on('connection', function (socket) {
            ...
            await mutex.runExclusive(async () => {
                ...
                if (background) {
                    ...
                    emitter.once('started', (out) => {
                        console.log(`${chalk.green(`[${socket.id}]`)} : [bus.run] : background job started: ID: ${out._id}, Name: ${out.name}, Host: ${out.host}, StartedAt: ${new Date(out.started).toISOString()}`);
                    });
                    ...
                    emitter.once('complete', (out) => {
                        console.log(`${chalk.green(`[${socket.id}]`)} : [bus.run] : background job completed: ID: ${out._id}, Name: ${out.name}, Host: ${out.host}, CompletedAt: ${new Date(out.completed).toISOString()}`);
                    });
        ...
        */
        socket.on('queue', async ({ name, host = null, background = true, options, description, exclusiveId = null }, callback = () => { }) => {
            ...
            if (!name) {
                callback(new Error('Job Name not Defined (QUEUE)'));
                return;
            }
            ...
            if (background) {
                ...
                emitter.once('started', (out) => {
                    console.log(`${chalk.green(`[${socket.id}]`)} : [bus.queue] : background job started: ID: ${out._id}, Name: ${out.name}, Host: ${out.host}, StartedAt: ${new Date(out.started).toISOString()}`);
                });
                emitter.once('error', (out) => {
                    console.error(`${chalk.green(`[${socket.id}]`)} : [bus.queue] : background job error: ID: ${out._id}, Name: ${out.name}, Host: ${out.host}`, out.error);
                });
                emitter.once('complete', (out) => {
                    console.log(`${chalk.green(`[${socket.id}]`)} : [bus.queue] : background job completed: ID: ${out._id}, Name: ${out.name}, Host: ${out.host}, CompletedAt: ${new Date(out.completed).toISOString()}`);
                });

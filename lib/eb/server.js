...
*/
    ...
    workerSpace.on('connection', function (socket) {
        ...
        socket.on('get', async (data, callback = () => { }) => {
            ...
            await mutex.runExclusive(async () => {
                ...
                // check if there are running exclusive jobs
                ...
                let pendingJob = await self.db.job_queue.findOne({
                    ...
                    exclusiveId: { $nin: exclusiveRunningJobs.map((j) => j.exclusiveId) }
                });

                // no pending jobs
                if (!pendingJob) {
                    // but some exclusive jobs
                    if (exclusiveRunningJobs.length) {
                        const remainingJobs = await self.db.job_queue.find({
                            type: TYPE_QUEUE,
                            serverHash,
                            status: STATUS_PENDING,
                            host: {
                ...
                if (!worker)
                    throw Error(`Worker Node lookup failed with socketId: ${socket.id}`);
        ...
        socket.on('background-in-progress', async (job, callback = () => { }) => {
            ...
            console.log(`${chalk.green(`[${socket.id}]`)} : [worker.background-in-progress] background in progress for Job: ${job._id} Type: ${job.type}`);

            if (!queue.has(job._id)) {
                await self.db.job_queue.delete(job);
                console.error(`${chalk.green(`[${socket.id}]`)} : [worker.background-in-progress] : EventEmitter not available! Job id '${job._id}' not found in queue map.`);
                return callback();
            }
        ...
        socket.on('done', async (job, callback = () => { }) => {
            ...
            console.log(`${chalk.green(`[${socket.id}]`)} : [worker.done] is done with Job: ${job._id} Type: ${job.type}`);

            if (!queue.has(job._id)) {
                await self.db.job_queue.delete(job);
                console.error(`${chalk.green(`[${socket.id}]`)} : [worker.done] : EventEmitter not available! Job id '${job._id}' not found in queue map.`);
                return callback();
            }
    ...
    */
    ...
    busSpace.on('connection', function (socket) {
         ...
         */
        socket.on('run', async ({ name, host, background, options, description }, callback = () => { }) => {

            console.log(`${chalk.green(`[${socket.id}]`)} : [bus.run] : new job: '${name}'`, description ? description : '', host ? `(Host: ${host})` : '');

            if (background) {
                console.warn(`${chalk.green(`[${socket.id}]`)} : ${chalk.red.bold('Please use the job type \'queue\' for background job execution!')}`);
            }
            ...
            await mutex.runExclusive(async () => {
                ...
                console.log(`${chalk.green(`[${socket.id}]`)} : [bus.run] : run job ${name} on worker: ${worker.socketId}`);
                ...
                if (background) {
                    emitter.once('setup-error', (out) => {
                        console.error(`${chalk.green(`[${socket.id}]`)} : [bus.run] : background job setup error: ID: ${out._id}, Name: ${out.name}, Host: ${out.host}`, out.error);
                    });
                    emitter.once('started', (out) => {
                        console.log(`${chalk.green(`[${socket.id}]`)} : [bus.run] : background job started: ID: ${out._id}, Name: ${out.name}, Host: ${out.host}, StartedAt: ${out.started.toUTCString()}`);
                    });
                    emitter.once('error', (out) => {
                        console.error(`${chalk.green(`[${socket.id}]`)} : [bus.run] : background job error: ID: ${out._id}, Name: ${out.name}, Host: ${out.host}`, out.error);
                    });
                    emitter.once('complete', (out) => {
                        console.log(`${chalk.green(`[${socket.id}]`)} : [bus.run] : background job completed: ID: ${out._id}, Name: ${out.name}, Host: ${out.host}, CompletedAt: ${out.completed.toUTCString()}`);
                    });
                ...
                try {
                    ...
                    console.error(`${chalk.green(`[${socket.id}]`)} : [bus.run] : failed to execute ${job._id} in worker ${worker._id}`, e);
                    queue.delete(job._id);
                    throw e;
                }
            });
        ...
        */
        socket.on('queue', async ({ name, host, background, options, description, exclusiveId = null }, callback = () => { }) => {
            console.log(`${chalk.green(`[${socket.id}]`)} : [bus.queue] : new job: '${name}'`, description ? description : '', host ? `(Host: ${host})` : '');
            ...
            if (background) {
                emitter.once('setup-error', (out) => {
                    console.error(`${chalk.green(`[${socket.id}]`)} : [bus.queue] : background job setup error: ID: ${out._id}, Name: ${out.name}, Host: ${out.host}`, out.error);
                });
                emitter.once('started', (out) => {
                    console.log(`${chalk.green(`[${socket.id}]`)} : [bus.queue] : background job started: ID: ${out._id}, Name: ${out.name}, Host: ${out.host}, StartedAt: ${out.started.toUTCString()}`);
                });
                emitter.once('error', (out) => {
                    console.error(`${chalk.green(`[${socket.id}]`)} : [bus.queue] : background job error: ID: ${out._id}, Name: ${out.name}, Host: ${out.host}`, out.error);
                });
                emitter.once('complete', (out) => {
                    console.log(`${chalk.green(`[${socket.id}]`)} : [bus.queue] : background job completed: ID: ${out._id}, Name: ${out.name}, Host: ${out.host}, CompletedAt: ${out.completed.toUTCString()}`);
                });
            ...
            try {
                ...
                console.error(`${chalk.green(`[${socket.id}]`)} : [bus.queue] : failed to 'run' queue job ${job._id}`, e);
                queue.delete(job._id);
                throw e;
            }

...
module.exports = function () {
    ...
    workerSpace.on('connection', function (client) {
        ...
        client.on('done', (inboundJob, callback) => {
            ...
            if (job.error !== undefined) {
                ...
                job.status = 'complete';
                job.emitter.emit('complete', { result: job.result, host: inboundJob.host, platform: inboundJob.platform, id: inboundJob.id });
            }
    ...
    busSpace.on('connection', function (client) {
        ...
        client.on('run', ({name, host, options}, callback) => {
            console.log(client.id, 'has added a new job (/bus/run). Job:', name);

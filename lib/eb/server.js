...
module.exports = async function () {
    ...
    const serverHash = crypto.createHash('md5').update(ipAddresses.join('')).digest('hex');
    ...
    setInterval(removeDisconnected, 70000);
    ...
    */
    ...
    workerSpace.on('connection', function (client) {
        ...
        client.on('register', async (data, callback) => {
            ...
            if (data.oldSocketId != -1) {
                ...
                */
                const oldWorker = await self.db.worker_node.findOne({ id: data.oldSocketId });
                if (oldWorker) {
            ...
            const worker = await self.db.worker_node.findOne({ id: data.id });
            if (!worker)
                await self.db.worker_node.insert({
                    id: data.id,
                    serverHash,
                    host: data.host,
            ...
            console.log(`[worker.register] ${data.id} has connected. Total connected nodes: ${workerNodes.length}`);
        ...
        client.on('get', async (data, callback) => {
            ...
            const pendingJob = await self.db.queue_job.findOne({
                type: TYPE_QUEUE,
            ...
            const worker = await self.db.worker_node.findOne({ id: client.id });
            if (!worker)
                throw Error(`Worker Node lookup failed with ID: ${client.id}`);

            await self.db.queue_job.update({
                _id: pendingJob._id,
            ...
            await self.db.worker_node.update({ _id: worker._id, assignedJobs: worker.assignedJobs + 1 });

            if (typeof callback === 'function') {
                return callback({
                    id: pendingJob.id,
                    name: pendingJob.name,
                    background: pendingJob.background,
                    options: pendingJob.options
                });
            }

        });

        client.on('background-in-progress', async (inboundJob, callback) => {
            console.log(`[worker.background-in-progress] ${client.id} background in progress for Job: ${inboundJob.id} Type: ${inboundJob.type}`);

            if (!queue.has(inboundJob.id)) {
                console.error(`'background-in-progress' job ${inboundJob.id} not found in queue map.`);
                await self.db.queue_job.delete({ _id: inboundJob.id });
            }


            const job = await self.db.queue_job.findOne({ _id: inboundJob.id });
            ...
            if (job.error !== undefined) {
                ...
                queue.get(job._id).emit('complete', { result: job.result, status: job.status, id: job.id, type: job.type, host: inboundJob.host, platform: inboundJob.platform, background: inboundJob.background });
            ...
            const workerNode = await self.db.worker_node.findOne({ id: client.id });

            await self.db.queue_job.update({ _id: job._id, result: inboundJob.result, error: inboundJob.error, workerId: workerNode._id, completed: Date.now() });

            if (typeof callback === 'function')
                return callback();
        });

        client.on('done', async (inboundJob, callback) => {
            console.log(`[worker.done] ${client.id} is done with Job: ${inboundJob.id} Type: ${inboundJob.type}`);

            if (!queue.has(inboundJob.id)) {
                console.error(`'done' job ${inboundJob.id} not found in queue map.`);
                await self.db.queue_job.delete({ _id: inboundJob.id });
            }
        ...
        client.on('disconnect', async (reason) => {
            ...
            console.log(`[worker.disconnect] ${clientId} has disconnected. Reason: '${reason}'`);

            const disconnectedWorker = await self.db.worker_node.update({ id: clientId, disconnected: true });
            if (disconnectedWorker) {
                console.log(`[worker.disconnect] Worker marked as disconnected: ${disconnectedWorker.id}`);
            } else {
            ...
            const runningJobs = await self.db.queue_job.find({ workerId: disconnectedWorker._id, status: STATUS_IN_PROGRESS });
            ...
            setTimeout(async (_id) => {
                ...
                await Promise.all(runningJobs.map(async (job) => {
                    ...
                    await self.db.queue_job.update({ _id: job._id, status: 'failed' });
                    if (job.background !== true) {
    ...
    busSpace.on('connection', function (client) {
        ...
        client.on('run', async ({ name, host, background, options, description }, callback) => {
            ...
            const job = await self.db.queue_job.insert({
                type: TYPE_EXE,
                name,
                options,
                host: worker.host,
            ...
            /*
                workerSpace.to(worker.id).emit('exe', job, (error, result) => {
        ...
        */
        ...
        client.on('housekeeping', async ({ options, host }, callback) => {
            ...
            if (hostWorker) {
                console.log(`execute housekeeping on '${host}' with worker ${hostWorker.id}`);
    ...
    return {
        getWorkerNodes: async () => {
            return await self.db.worker_node.find({});
        },
        getJobs: async () => {
            return await self.db.queue_job.find({ type: TYPE_QUEUE }, (query) => query.sort('-created'));
        },
        getExeJobs: async () => {
            return await self.db.queue_job.find({ type: TYPE_EXE }, (query) => query.sort('-created'));

        },
        getJob: async (id) => {
            return await self.db.queue_job.find({ id });
        }
    };
};

...
module.exports = function () {
    ...
    if (cluster.isMaster) {
        ...
        setupMaster(ioServer, {
            loadBalancingMethod: 'least-connection', // either "random", "round-robin" or "least-connection"
        });
        console.log('listening on websocket port ', ebPort);
    ...
    */
    ...
    workerSpace.on('connection', function (client) {
        ...
        client.on('register', async (data, callback) => {
            ...
            if (data.oldSocketId != -1) {
                ...
                */
                const worker = await self.db.worker_node.findOne({ id: data.oldSocketId });
                if (worker) {
                    ...
                    await self.db.worker_node.update(worker);
                } else {
            ...
            const workerNodes = await self.db.worker_node.find({});
        ...
        client.on('get', async (data, callback) => {
            ...
            const nextPendingJob = await self.db.queue_job.findOne({
                type: TYPE_QUEUE,
            ...
            await self.db.worker_node.update({ _id: worker._id, assignedJobs: worker.assignedJobs++ });
        ...
        client.on('background-in-progress', async (inboundJob, callback) => {
            ...
            await self.db.queue_job.update(job);
    ...
    busSpace.on('connection', function (client) {
        ...
        client.on('run', async ({ name, host, background, options, description }, callback) => {
            ...
            const workerNodes = await self.db.worker_node.find({ disconnected: false });

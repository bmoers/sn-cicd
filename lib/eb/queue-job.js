 ...
 * @param {String} opt.background [true] don't wait for the job to resolve (use this with care!)
 * @param {String} opt.description give it a reasonable description
 * @param {String} opt.exclusiveId if set, the job is exclusive. new jobs with the same exclusiveId wait for previous job to complete
 * @param {String} opt.exclusive mark this job type as exclusive (uses 'name' as exclusiveId)
 * @param {Any} options The options to be passed to the job-module
 * @returns {Promise} The 'job' promise with the job result
 */
module.exports = function ({ name, host, background = true, description, exclusiveId, exclusive = false  }, options, logger) {
    ...
    return new Promise((resolve, reject) => {
        if (!name)
            return reject('[QueueJob] Job name not defined');

        logger.log('[QueueJob] Add Job:', name);

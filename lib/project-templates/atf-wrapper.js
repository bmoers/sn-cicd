...
describe("Execute ATF: Wrapper", function () {
    ...
    return getCommitId().then((commitId) => {
        ...
        if (!results.suiteResults || !results.suiteResults || ((results.suiteResults.length === 0 || (results.suiteResults[0].testResults && results.suiteResults[0].testResults.length === 0)) && results.testResults.length === 0)) {

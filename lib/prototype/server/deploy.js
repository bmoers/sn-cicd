...
module.exports = function () {
    ...
    router.post(DEPLOY_EXECUTE, (req, res) => {
        ...
        return deploymentWrapper.run.call(self, { commitId, from, to, deploy, git }).then(({ commitId, id }) => {
            console.log('redirect to ', `/deploy${DEPLOY_EXECUTE}/${commitId}/${id}`);
            res.setHeader('Location', `/deploy${DEPLOY_EXECUTE}/${commitId}/${id}`);
     ...
     */
    router.get(`${DEPLOY_EXECUTE}/:commitId/:id`, (req, res) => {
        return deploymentWrapper.get.call(self, { commitId: req.params.commitId, id: req.params.id }).then((results) => {

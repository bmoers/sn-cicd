    ...
    router.get(`${UPDATE_SET}/:deploymentId`, (req, res) => {
        ...
        */
        return self.db.deployment.findOne({
            ...
            const runUpdateSet = run.config.updateSet;
            ...
            */
            const sysId = deployment.sysId; //require('crypto').createHash('md5').update(scopeName.concat(run.commitId)).digest('hex');
            //console.log("Generated SYS_ID", sysId);
            ...
            */
            ...
            let description = `Installed via CICD \n >  Update set based on Commit ID #${run.commitId.substr(0, 7)} \n- - - - \n${updateSet.description}`
            if (mergedDeployment) {
                name = (updateSet.appVersion != "0") ? `${updateSet.appName} - ${updateSet.appVersion}` : updateSet.appName;
                if (updateSet.name != name) {
                    name = `${updateSet.name} (${(updateSet.appVersion != "0") ? `${scopeName} - ${updateSet.appVersion}` : scopeName})`;
                }
                // append short commit hash to have an unique name
                name = `${name} #${run.commitId.substr(0, 7)}`

                const commitIds = deployment.scope.commitIds.map((id) => {
                ...
                const artifacts = deployment.scope.artifacts.map((ar) => `${ar.split(/[\/\\]/).pop()}`).join('\n \t- ');

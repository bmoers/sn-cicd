...
module.exports = function () {
    ...
    router.get(`${UPDATE_SET_XML_COUNT}/:deploymentId`, (req, res) => {
        ...
        return self.db.deployment.findOne({
            ...
            return new EbQueueJob({ name: 'extractUpdateSet', background: false, host: deployment.host, exclusiveId: `extract-${deployment._id}` }, { deploymentId: deployment._id, count: true }).then((job) => {
    ...
    router.post(`${UPDATE_SET_XML_DATA}/`, (req, res) => {
        ...
        return self.db.deployment.findOne({
            ...
            return new EbQueueJob({ name: 'extractUpdateSet', background: false, host: deployment.host, exclusiveId: `extract-${deployment._id}` }, { deploymentId: deployment._id, xmlSysIds }).then((job) => {

...
module.exports = async function () {
    ...
    return Promise.try(() => {
            ...
            return coll.findAsync({ __migrated: { $exists: false } }).then((projectList) => {
                ...
                return Promise.each(projectList, (project) => {
                    ...
                    return projectModel.insertAsync(project).then(() => {
                        insertsNum++
                        return coll.updateAsync({ _id: project._old_id }, { $set: { __migrated: true } });
                    });
        ...
        const target = path.resolve(self.settings.dataStore.path, '../db_archive');

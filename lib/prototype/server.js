...
module.exports = function () {
    ...
    const isPortAvaliable = (port) => new Promise((resolve, reject) => {
        if (!port)
    ...
    return self.init(self.SERVER).then(() => {
        ...
        return isPortAvaliable(httpPort).then((free) => {
            if (!free)
                throw Error(`Port ${httpPort} is already in use`);
        }).then(() => {
            if (secure)
                return isPortAvaliable(httpsPort).then((free) => {
                    if (!free)
        ...
        const server = (() => {
            if (secure) {
                ...
                redirect.get('*', function (req, res) {
                    res.redirect(`https://${req.headers.host.split(':')[0]}:${serverPort}req.url`);
                });
        ...
        // get the status of the exoort job

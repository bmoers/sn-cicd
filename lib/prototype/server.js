...
module.exports = function () {
    ...
    return self.init(self.SERVER).then(() => {
        ...
        app.get('/monitor', (req, res) => {
            res.json({ pid: process.pid, cluster: {  } });
        });

        app.get('/test', async (req, res) => {
            const EventBusJob = require('../eb/job');
            //const HouseKeepingJob = require('../eb/housekeeping');
            //const j = await new HouseKeepingJob({ codeDir: '/tmp/gulz/bulz' }, 'localhost');
            //const j = new EbQueueJob({ name: 'hello', background: true, description: 'Hello World' }, {a:1});
            const j = new EventBusJob({ name: 'hello', background: true, description: 'Hello World' }, {a:1});
            res.json({ j });
        });

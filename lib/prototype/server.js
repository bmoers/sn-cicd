...
module.exports = function () {
    ...
    return self.init(self.SERVER).then(() => {
        ...
        if ('mongo' == self.settings.dataStore.type && 'true' == process.env.CICD_DB_MONGO_EXPRESS_UI_ENABLED) {
            ...
            var config = require('mongo-express/config.default')
            ...
            if (process.env.CICD_DB_MONGO_EXPRESS_ADMIN_USER && process.env.CICD_DB_MONGO_EXPRESS_ADMIN_PASSWORD) {
                ...
                const prsHost = /^(mongodb:(?:\/{2})?)(([^:]+?):([^@]+?)@|:?@?)([^:@]+?)(:(\d+))?\/(\w+?)$/;
                const match = prsHost.exec(process.env.CICD_DB_MONGO_URL);
                const server = match[5];
                const port = match[7];
                config.mongodb.server = server;
                config.mongodb.port = port;
                config.mongodb.adminUsername = process.env.CICD_DB_MONGO_EXPRESS_ADMIN_USER;
                config.mongodb.adminPassword = process.env.CICD_DB_MONGO_EXPRESS_ADMIN_PASSWORD;

            } else {
                // admin is disabled, only access the the CICD DB
                config.mongodb.connectionString = process.env.CICD_DB_MONGO_URL;
            }

            // secure the mongo_express UI. random data used if no credentials specified
            config.basicAuth = {
                username: process.env.CICD_DB_MONGO_EXPRESS_UI_USER === undefined ? Math.random().toString() : process.env.CICD_DB_MONGO_EXPRESS_UI_USER, 
                password: process.env.CICD_DB_MONGO_EXPRESS_UI_PASSWORD === undefined ? Math.random().toString() : process.env.CICD_DB_MONGO_EXPRESS_UI_PASSWORD
            }

            // disable logging
            config.options.console = false;
            config.options.logger = { skip: () => true }

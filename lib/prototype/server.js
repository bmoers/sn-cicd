...
module.exports = function () {
    ...
    return self.init(self.SERVER).then(() => {
        ...
        app.get('/eb/jobs', async (req, res) => {
            res.json((await eventBusServer.getJobs()));
        });
        app.get('/eb/exe', async (req, res) => {
            res.json((await eventBusServer.getExeJobs()));
        });
        ...
        app.route('/run').post((req, res) => {
            return this.convertBuildBody(req.body).then((options) => {
                ...
                return self.db.us.findOne({
                    ...
                    return new EbQueueJob({ name: 'run', background: true, description: `Build UpdateSet ${options.updateSet}` }, options).then((job) => {
                        return res.json({
                            run: 'added-to-queue',
                            result: job,
                            status: `/run/${job._id}`
        ...
        app.route('/preview-complete').post((req, res) => {
            ...
            return self.db.run.findOne({
                ...
                return Promise.try(() => {
                    ...
                    return new EbQueueJob({ name: 'run', background: true, description: `Build UpdateSet ${run.config.updateSet.sys_id}` }, run.config).then((job) => {
                        return res.json({
                            result: job,
                            url: `/goto/run/${run._id}`
                        });
        ...
        app.route('/export').post((req, res) => {
            return this.convertBuildBody(req.body).then((options) => {
                ...
                return self.db.us.findOne({
                    ...
                    return new EbQueueJob({ name: 'export', background: true, description: `Export UpdateSet ${options.updateSet}` }, options).then((jobs) => {
                        return res.json({

    ...
    return self.init(self.SERVER).then(() => {
        ...
        app.route('/deployment-complete').post((req, res) => {
            ...
            const payload = assign({
                ...
                isInteractive: null, // true == via UI, false == REST
                user: {
                    name: null,
                    fullName: null,
                    email: null
            ...
            return self.db.deployment.findOne({
                ...
                if (!deployment) {
                    console.warn(`Deployment not found with 'remoteUpdateSetID' ${payload.remoteUpdateSet}`)
                    return res.send(`Deployment not found with 'remoteUpdateSetID' ${payload.remoteUpdateSet}`);
                }
                ...
                return self.db.run.find(deployment.runId).then((run) => {
                    if (!run)
                        return res.send(`Run not found with 'id' ${deployment.runId}`);

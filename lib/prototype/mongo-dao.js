...
Schema.Types.ObjectId.get(v => v ? v.toString() : v);
...
const updateSetSchema = new Schema({
    "_id": { type: Schema.Types.ObjectId, default: ObjectID },
    "appId": { type: Schema.Types.ObjectId, ref: 'application', index: true },//{ type: String, index: true },
    ...
    "uuid": String,
    "runId": { type: Schema.Types.ObjectId, ref: 'run', index: true },//{ type: String, index: true },
    "pullRequestRaised": Boolean
}, schemaOptions);

const runSchema = new Schema({
    "_id": { type: Schema.Types.ObjectId, default: ObjectID },
    "appId": { type: Schema.Types.ObjectId, ref: 'application' },//String,
    "usId": { type: Schema.Types.ObjectId, ref: 'us', index: true },//{ type: String, index: true },
    "testId": { type: Schema.Types.ObjectId, ref: 'test' },//String,
    "sequence": Number,
...
const deploymentSchema = new Schema({
    "_id": { type: Schema.Types.ObjectId, default: ObjectID },
    "appId": { type: Schema.Types.ObjectId, ref: 'application', index: true },//{ type: String, index: true },
    "usId": { type: Schema.Types.ObjectId, ref: 'us' },//String,
    "runId": { type: Schema.Types.ObjectId, ref: 'run' },//String,
    "state": String,
    "commitId": { type: String, index: true },
    "sysId": String,
    "remoteUpdateSetID": { type: String, index: true },
    ...
    const getOperations = (table) => {
        ...
        return {
            ...
            insert: (obj) => {
                ...
                //return model.create([obj], { lean: true }).then((r) => r[0]);
            },
            /**
             * update an object
            ...
            find: (query) => {
                return model.find(query).lean().exec(); //(r) => r.map((o) => o.toJSON())
            },
            findOne: (query) => {
                return model.findOne(query).then((r) => {
                    console.log("----------- findOne")
                    console.dir(r, { colors: true })
                    console.log("-----")
                    console.dir(r.toJSON(), { colors: true })
                    console.log("-----------")
                    console.dir(r.x(), { colors: true })
                    return r;
                }); // .exec((r) => r.toJSON())
            }
        };

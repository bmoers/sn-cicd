...
module.exports = function ({ id = uuidv4().toLowerCase(), commitId, from, to, deploy, git }, logger = console, { host }) {
    ...
    */
    ...
    return self.db.deployment.findOne({
        ...
        return Promise.try(() => {
            return step(`${deploy ? 'Deploying' : 'Delivering'} ${deployment.scope.commitIds.length} commits in scope '${deployment.scopeName}' to '${targetHostName}' ${deployFromGit ? 'via GIT' : ''}`);
        }).then(() => {
            // Update-Set <${self.link(sourceHostName, `/sys_update_set.do?sys_id=${config.updateSet.sys_id}`)}|${config.updateSet.name}>
            return slack.build.start(`*${run.config.application.name} › ${run.config.updateSet.name} › #${run.sequence}*\nDEPLOYMENT - In progress.\n\n${deploy ? 'Deploying' : 'Delivering'} ${deployment.scope.commitIds.length ? `${deployment.scope.commitIds.length} commits` : 'one commit'} in scope '${deployment.scopeName}' from <${sourceHostName}|${sourceHostName}> to <${targetHostName}|${targetHostName}> ${deployFromGit ? 'via GIT' : ''}\n\n<${run.config.application.docUri}|details>`);

...
module.exports = function (runId, logger = console, { host }) {
    ...
    return Promise.try(() => {
                ...
                */
                ...
                return git.merge('master').catch(() => {
                    return Promise.try(() => {
                        ...
                        return step("reset HARD from master");
                    }).then(() => {
                        return git.exec({
                            quiet: false,
    ...
    }).then(() => { // export update-set-xml as file
        return Promise.try(() => {
            ...
            return Promise.try(() => {
                ...
                return git.add(fileName).then(() => {
                    return git.commit({
                        ...
                        messages: [`${config.updateSet.name} - UpdateSet XML File - Build #${config.build.sequence}`]
                    });
                });
            });
        });
        ...
        return Promise.try(() => {
            ...
            return client.getUpdateSetFiles(config.updateSet.sys_id, (results) => {
                ...
                return step(`all files locally created/removed form update-set-xml. UpdatedFiles: ${updatedFiles.length}, DeletedFiles: ${deletedFiles.length}`);
            });
        ...
        return Promise.try(() => {
            ...
            */
            return self.getFilesByUpdatedBy(touchedFiles.updatedFiles).then((fileByUpdatedBy) => { // sort updated files update by user

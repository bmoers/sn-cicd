            ...
            if (config.git.enabled && config.git.pullRequestEnabled) {
                return Promise.try(() => {
                    ...
                    return self.raisePullRequest({
                        ...
                        title: `${config.updateSet.name} (${config.build.requestor.fullName}) #${config.build.sequence}`,
                ...
                return deploymentWrapper.run.call(self, { commitId: run.commitId, deploy: true });
                //return new EventBusJob({ name: 'deployUpdateSet' }, { commitId: run.commitId, deploy: true }, logger);
            } else {

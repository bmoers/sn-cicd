        ...
        if (run.buildPass) {
            return Promise.try(() => {
                ...
                if (config.git.enabled && config.git.pullRequestEnabled) {
                    return Promise.try(() => {
                        return step(`set update-set status to ${self.build.CODE_REVIEW_PENDING}`);
                    }).then(() => {
                        ...
                        return step('pull request raised');
                    }).then(() => {
                        return self.db.us.findOne({ runId: run._id }).then((us) => {
                            if (us) {
                                ...
                                logger.warn('related update set not found with runId', run._id);
                            }
                        }).then(() => {
                    ...
                    return self.setRunState(run, self.run.SUCCESSFUL).then(async () => {
                        ...
                        */
                        const job = await new EbQueueJob({ name: 'deploy', background: true, exclusiveId: `deploy-${run.commitId}`, description: `Deploy ${config.application.name} : ${config.updateSet.name} : ${run.commitId}` }, 

        ...
        return Promise.try(() => {
            ...
            return Promise.try(() => {
                ...
                return Promise.try(() => {
                    ...
                    const job = await new EbQueueJob({ name: 'deploy', background: true, exclusiveId: `deploy-${config.build.commitId}`, description: `Deploy ${config.application.name} : ${config.updateSet.name} : ${config.build.commitId}` },

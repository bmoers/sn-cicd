...
module.exports = function ({ body }, logger = console, { host }) {
    ...
    if (!decline && !merge && !deleted) {
        return Promise.try(() => {
            return logger.info(`Ignore. irrelevant action '${action}'`); // only handle known actions
        });
    }

    if (!body.action || !body.target || !body.target.branch)
        return Promise.try(() => {
            return logger.warn('seems not to be a valid pull request inbound:', body);
        });
    ...
    return Promise.try(() => {
        ...
        return Promise.try(() => {
            ...
            return Promise.try(() => {
                if (merge && !body.mergeId) {
                    ...
                    */
                    ...
                    const tmpDir = path.join(config.application.dir.tmp, uuidv4());
            ...
            return Promise.try(() => {
                ...
                if (config.deploy && config.deploy.enabled && config.deploy.onPullRequestResolve)
                    return step(`complete update-set ${config.updateSet.name}`).then(() => {
                ...
                return Promise.try(() => {
                    ...
                    // return deploymentWrapper.run.call(self, { commitId: config.build.commitId, deploy: true }, logger);

                    const job = await new ExeJob({ name: 'deploy', background: true, exclusiveId: `deploy-${config.build.commitId}`, description: `Deploy ${config.application.name} : ${config.updateSet.name} : ${config.build.commitId}` },
                        { commitId: config.build.commitId, deploy: true }, logger);

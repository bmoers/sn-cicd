...
module.exports = async function ({ commitId, from, to, deploy, git = (process.env.CICD_CD_DEPLOY_FROM_GIT === 'true') }, logger = console, { host, exclusiveId }) {
    ...
    */
    ...
    return Promise.mapSeries(deployments, (deploymentJob) => {
        return new EbQueueJob({ name: 'deployUpdateSet', background: true, exclusiveId: deploymentJob.scopeName }, { id: deploymentJob._id, commitId, from: sourceHostName, to: targetHostName, deploy, git }, logger).then((job) => {

        ...
        return Promise.try(() => {
            ...
            */
            ...
            if (config.updateSet.application !== undefined) {
                console.log(`CICD run on a scoped app. Convert app '${config.updateSet.application}' to an update-set`)
            ...
            return getUs.call(self, { appId: config.build.applicationId, updateSet: config.updateSet }).then((_us) => {
                ...
                console.log(`Checking for Fix Deployment with pattern '${pattern}'`);
                const match = (() => {
                    const reg = pattern.match(/^\/(?<pattern>.*)\/(?<flags>[gmiyus]*$)/);
                    if (reg) {
                        console.log(`RegExp Pattern: ${reg.groups.pattern}, Flags: ${reg.groups.flags}`)

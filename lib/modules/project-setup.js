...
module.exports = function (options = {}, logger = console, { host }) {
    ...
    /*
        source: { // valid for e.g. 30 days !
                name: null,
                accessToken: null
            },
            master: { // valid for e.g. 30 mins
                name: null,
                accessToken: null
            },
            target: { // valid for e.g. 30 days !
                name: null,
                accessToken: null
            }
    */
    ...
    return self.db.us.find({
        ...
        return Promise.try(() => {
            if (result && result.length) {
                ...
                const us = assign({}, defaults, result[0], { // always refresh the update-set
                    updateSet: updateSet,
                    name: updateSet.name
                });
...
const configure = function (config = {}, logger = console) {
    ...
    return Promise.try(() => {
        ...
        return getApplication.call(self, config.application).then((_app) => {
            ...
            //return self.db.application.update(ctx.app);
        });
        ...
        config.application.dir = {
            code: path.resolve(self.settings.gitRepoRootDir, config.application.id, (config.build.sequence).toString()),
            doc: path.resolve(self.settings.documentsRootDir, config.application.id, (config.build.sequence).toString()),
            tmp: path.resolve(self.settings.tempBuildRootDir, config.application.id)
        };
        ...
        return Promise.try(() => {
            ...
            /*
            ctx.us.branchName = config.branchName;
            return self.db.us.update(ctx.us);
            */
        }).then(() => {
        ...
        return step(`Setup Project '${config.application.name}' on disc '${config.application.dir.code}'`).then(() => {
            return self.getProject(config).setup();
        });
    }).then(() => {

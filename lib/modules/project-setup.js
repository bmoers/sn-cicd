...
module.exports = function (options = {}, logger = console, { host }) {
    ...
    const configure = function (config = {}, logger = console) {
        ...
        return Promise.try(() => {
            ...
            console.log(self.settings.documentsRootDir, app._id, us.updateSetId, (run.sequence).toString());
            ...
            config.application.dir = {
                ...
                doc: path.resolve(self.settings.documentsRootDir, app._id, us.updateSetId, (run.sequence).toString()), // , (config.build.sequence).toString()
                tmp: path.resolve(self.settings.tempBuildRootDir, us.uuid),
                web: [app._id, us.updateSetId, (run.sequence).toString()].join('/')
            };

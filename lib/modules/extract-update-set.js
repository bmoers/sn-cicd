...
module.exports = function ({ commitId, count, xmlSysIds }, logger = console, { host }) {
    ...
    return self.db.run.findOne({
        ...
        const tmpDir = path.join(config.application.dir.tmp, commitId);
        const file = path.resolve(tmpDir, 'us', config.updateSet.scopeName, `sys_update_set_${updateSetSysId}.xml`);

        return fs.pathExists(file).then((exists) => {
            if (exists)
                return step(`File is already checked out`);

    ...
    return self.db.deployment.findOne({
        ...
        return fs.pathExists(scopeDir).then((exists) => { // clone repository if not exists
            ...
            if (mergedDeployment && baselineCommitId) {
                return git.listFiles(`${scopeDir}/*.xml`, `${baselineCommitId}...${commitId}`);
            } else {
                return [artifact];
            }
        ...
        if (mergedDeployment)
            console.log('updateSetFiles to be merged:', updateSetFiles);
        ...
        if (count) { // only return the count of sys_ids 
            ...
            const sysIds = fileChangeArray.map((xml) => xml.sys_id);
            const out = {
                sys_id: sysIds.join(','),
        ...
        */
        ...
        return Promise.all(batchPromises).then((batchResults) => { // wait for all parser to end
            ...
            const result = [].concat.apply([], batchResults).sort((a, b) => (parseInt(a.sys_recorded_at, 16) - parseInt(b.sys_recorded_at, 16))).map((element) => {
                if (!forcedDeployment)
                    return element;

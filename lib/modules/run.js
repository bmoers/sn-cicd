...
module.exports = async function (options, logger = console, { host }) {
    ...
    try {
        ...
        if (run) {
            ...
            // run new job
            const jobResult = await new EventBusJob({ name: 'projectSetup' }, options, logger);
        ...
        ({ result: run } = await new EventBusJob({ name: 'runCollisionDetection', host: buildOnHost }, runId, logger));
        ...
        await new EventBusJob({ name: 'exportFilesFromMaster', host: buildOnHost }, runId, logger);
        await new EventBusJob({ name: 'exportUpdateSet', host: buildOnHost }, runId, logger);

        if (process.env.CICD_EMBEDDED_BUILD === 'true') {
            await new EventBusJob({ name: 'buildProject', host: buildOnHost }, runId, logger);
        } else {

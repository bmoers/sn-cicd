                ...
                return Promise.try(() => {
                    if (removedFiles.length) {
                        return Promise.try(() => {
                            ...
                            return git.commit({
                                messages: [`'${removedFiles.length}' files removed on ${config.master.host.name}`, `no-cicd`]
                            });
                        });
                    }
                }).then(() => {
                    if (modifiedFiles.length) {
                        return Promise.try(() => {
                            ...
                            return git.commit({
                                messages: [`'${config.master.name}' branch updated from  ${config.master.host.name}`, `no-cicd`]
                            });
                        });
                    }
                }).then(() => {

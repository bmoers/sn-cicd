...
module.exports = function (defaults) {
    ...
    var pull = function (action, config) {
        ...
        var options = objectAssignDeep({
            request: {
                id: null,//"9",
                name: null,//"us/exampledev.service-now.com",
                url: null,//"https://git.example.com/projects/SNOW/repos/update-set/pull-requests/9/overview",
            },
            author: {
                id: null,//"U3VJB39K3",
            ...
            source: {
                project: null,//"SNOW",
                repository: null,//"update-set",
                branch: null,//"exampledev.service-now.com"
            },
            target: {
                project: null,//"SNOW",
                repository: null,//"update-set",
        ...
        options.actionText = options.action == DELETE ? 'deleted' : options.action == OPEN ? 'opened' : options.action == DECLINE ? 'declined' : options.action == MERGE ? 'merged' : '';
        options.color = options.action == DELETE ? '#FF0000' : options.action == OPEN ? '#2267c4' : options.action == DECLINE ? '#990016' : options.action == MERGE ? '#1e8217' : '#FFCC00';

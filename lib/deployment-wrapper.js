 ...
 * 
 */
module.exports.run = function ({ commitId, from, to, deploy, git = (process.env.CICD_CD_DEPLOY_FROM_GIT === 'true') }, logger = console) {
    const self = this;
    let config = {};
    let appId;
    ...
    return self.db.run.findOne({
        ...
        */
        ...
        }).then(async () => { // get the baseline CommitId: the last successful deployment to the target environment

...
module.exports.run = function ({ commitId, from, to, deploy, git }, logger = console) {
    ...
    return self.db.run.findOne({
        ...
        */
        ...
        // wait somewhere between 1.5 and 15 seconds to avoid race condition for concurrent deployments
        const delay = randomInt(1500, 15000);
            ...
            return self.db.deployment.find({
                ...
                if (deployments.length) {
                    ...
                    if (elapsedSeconds > maxCheckSeconds)
                        throw Error(`Another '${deploy ? 'deployment' : 'delivery'}' job is already in progress and did not finish within ${maxCheckSeconds} sec`);

...
var CICD = (function () {
    ...
    var CICD = function (options) {
        ...
        self.settings = ObjectAssignDeep({
            dataStore: {
                type: 'nedb',
    ...
    CICD.prototype.test = function () {
        const self = this;
        console.log('yeeeeeeeeeeeeeeeeeeeeeeeeeaaahhh');
    };
    ...
    CICD.prototype.getSlack = function () {
        ...
        return new self.Slack(self.settings.slack);
    };
    ...
    CICD.prototype.setUpdateSet = function (config, entry, error) {
        const self = this;
        return Promise.try(() => {
            if (error) {
                return self.addStep(config, 'setUpdateSet-error', error);
            }
        }).then(() => {
            ...
            console.error('CICD.prototype.setUpdateSet', e);
        });
    ...
    CICD.prototype.loadUpdateSet = function (ctx) {
        ...
        return Promise.try(() => {
            ...
            return client.getUpdateSetDetails(config.updateSet)
                .then((result) => {
                    ...
                    if (result.length) {
                        ...
                        config.updateSet = {
                            ...
                            appName: self.getDisplayValue(us['application.name']),
                            scopeName: self.getDisplayValue(us['application.scope'])
                        };

                        ['name', 'description', 'state', 'remote_sys_id', 'sys_created_by', 'sys_created_on', 'sys_updated_by', 'sys_updated_on'].forEach((field) => {
                            config.updateSet[field] = self.getValue(us[field]);
                        });
                    } else {
            ...
            return client.getFilesFromTable(requestParam, (files) => {
                ...
                return Promise.each(files, (file) => {
                    ...
                    */
                    ...
                    file.____ = {
                        hostName: client.getHostName(), // getHostName()
                        className: className,
                        appName: appName,
                        scopeName: scopeName,
                        updatedBy: updatedBy,
    ...
    CICD.prototype.buildUpdateSetOnBranch = function (options, ctx) {
        ...
        var step = (message, error) => {
            return self.addStep(config, 'buildUpdateSetOnBranch: '.concat(message), error);
        };
        ...
        return Promise.try(() => {
            ...
            */
            if (ctx.config.branch.enabled) {
                ...
                ctx.remote = self.getClient(ctx.config.branch);
    ...
    CICD.prototype.buildUpdateSet = function (options, ctx) {
        ...
        var step = (message, error) => {
            return self.addStep(config, 'buildUpdateSet: '.concat(message), error);
        };
    ...
    CICD.prototype.exportUpdateSet = function (options, ctx) {
        ...
        var step = (message, error) => {
            return self.addStep(config, 'exportUpdateSet: '.concat(message), error);
        };
        ...
        return Promise.try(() => {
            ...
            /*
            // no idea why I created activeClient here....

            activeClient = self.getClient(ctx.config);
            activeClient = new SnClient({
                host_name: config.host.name,
                proxy: config.settings.proxy,
                client_id: config.host.credentials.oauth.clientId,
                client_secret: config.host.credentials.oauth.clientSecret,
            ...
            */
            ...
            return Promise.try(() => {
                ...
                return self.configureProjectFile(files, ctx).then(() => files);
            });
            ...
            return Promise.try(() => {
                ...
                return git.delete(files.deletedFiles.map((file) => file.path)).then(() => {
                    ...
                    return git.commit({
                        ...
                        messages: [`DELETED FILES: ${config.updateSet.name} - Build #${ctx.config.build.sequence}`, config.updateSet.description]
                    });
                });
                */
            }).then(() => {
                ...
                */
                ...
                /*
                    group files by user and commit individually
                */
                return self.getFilesByUpdatedBy(files.deletedFiles).then((fileByUpdatedBy) => { // sort deleted files update by user
                    ...
                    return Promise.each(Object.keys(fileByUpdatedBy), (updatedBy) => {
                        ...
                        return git.delete(fileByUpdatedBy[updatedBy]).then(() => {
                            return self.getUser(ctx.client, updatedBy);
                        }).then((user) => {
    ...
    CICD.prototype.deployUpdateSet = function (options, _ctx) {
        ...
        var step = (message, error) => {
            return self.addStep(config, 'deployUpdateSet: '.concat(message), error);
        };

        return Promise.try(() => {

            /*
                manually configure as the self 'this.configure()' method will cause the project to re-initialize
            */
            ctx.config.settings = ObjectAssignDeep({}, self.settings);
            ctx.client = client = self.getClient(ctx.config);

            ctx.config.deploy.settings = ctx.config.settings;
            ctx.target = self.getClient(ctx.config.deploy);
            ...
            return Promise.try(() => {
                ...
                return client.deployUpdateSet(config.updateSet.sys_id, ctx.config.deploy.host.name).catch((e) => {
                ...
                /*
                {
                    sourceSysId: 'b07ea76fdbaf1f00c398b94ffe96199a',
                    remoteUpdateSetSysId: 'ca5b505fdb3317c08ddafc600f96192c',
                    progressId: '535bd453dbb7d7c0c398b94ffe9619cd',
                    targetEnvironment: 'https://swissredevone.service-now.com/',
                    updateSetSysId: '3971107bdbefd300dfa9b94ffe9619e6',
                    step: 'deploymentComplete',
                    api: 'api',
                    state: 'committed',
                    sourceEnvironment: 'https://swissre1.service-now.com/',
                    targetUpdateSetSysId: '035b5cd3db77d7c0c398b94ffe9619d6'
                }
                */
                return step(`UpdateSet successfully transferred. State is committed`).then(() => {
                    return result;
                });
    ...
    CICD.prototype.gitPullRequestUpdate = function (body) {
        ...
        return slack.pullRequest.send(body).catch((e) => { // bypass the message to the slack channel
            ...
            return Promise.try(() => {
                ...
                config.settings = ObjectAssignDeep({}, self.settings);
                ctx.config = config;
    ...
    CICD.prototype.run = function (options, _ctx) {
        ...
        return Promise.try(() => {
        ...
        }).then(({id, result, host, platform}) => {
            //console.log('######################################## run() result', id, result, host, platform);
            ...
            return self.setUpdateSet.call(this, ctx.config, 'run()', error);
        });
        // init git (remote repo)

        // export dat from master

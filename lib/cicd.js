...
var CICD = (function () { 
        ...
        */
        ...
        return Promise.reduce(applicationFiles, (applicationFilesByClass, file) => {
            ...
            return Promise.each(Object.keys(applicationFilesByClass), (className) => {
                ...
                return Promise.each(chunks, (files, index) => {
                    console.log(`\t\tchunk #${index}`);
                    return processFiles(ctx, className, files).then((filesUpdated) => {
    ...
    CICD.prototype.deployUpdateSet = function (options, _ctx) {
        ...
        return Promise.try(() => {
            ...
            */
            ...
            ctx.target = getClient(ctx.config.deploy).client;
    ...
    CICD.prototype.server = function () {
        ...
        const jsonSave = (req, res, next) => {
            bodyParser.json({ verify: (req, res, buf, encoding) => { req.rawBody = buf.toString(); } })(req, res, (err) => {
                if (err) {
                    console.log(err);
                    res.sendStatus(400);
                    return;
        ...
        const defaultJson = [bodyParser.urlencoded({
            extended: false
        }), jsonSave];

        const jsonFixQuote = (req, res, next) => {
            if (/\/json$/.test(req.headers['content-type'])) {
                try {
                    req.body = JSON.parse(req.body.toString().replace(/(?<!\\)\\'/g, '\''));
                } catch (e) {
                    console.log(e);
        ...
        const fixJson = [bodyParser.raw({
            type: 'application/json'
        }), jsonFixQuote, bodyParser.urlencoded({
        extended: false
        })];
        ...
        app.post('/build', defaultJson, (req, res) => {
        ...
        app.post('/pull_request', fixJson, (req, res) => {

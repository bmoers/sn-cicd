...
var CICD = (function () { 
    ...
    var getDisplayValue = (element) => {
        if (element === undefined)
            return undefined;
        return element.display_value || element.value || element;
    };
    ...
    var configureProjectFile = function (files, ctx) {
        ...
        return Promise.try(() => {
            ...
            return project.getTestSuites().then((testSuites) => {
            ...
            return project.getTests().then((tests) => {
            ...
            //console.dir(testConfig, { depth: null, colors: true });
            return project.writeFile('config/project.json', JSON.stringify(testConfig, null, '\t'));
        });
    ...
    var setUpdateSet = function (config, entry, error) {
        ...
        return Promise.try(() => {
            ...
            if (config.build.run.us) {
                ...
                return this.db.us.update(config.build.run.us);
            }
        });
    ...
    CICD.prototype.db.us = {
        get: function ({ _id }) {
            return Promise.try(() => {
                return dataStore.us.findOneAsync({ _id });
            });
        },
        insert: function (obj) {
            return Promise.try(() => {
        ...
        return Promise.try(() => {
            ...
            if (!ctx.config.build.run.us) {
                return getUpdateSet.call(this, ctx.config).then((us) => {
                    ...
                    /*
                    us.host = ctx.config.host;
                    us.deploy = ctx.config.deploy;
                    us.requestor = ctx.config.build.requestor;
                    */
                    ctx.config.build.run.us = us;
                    ctx.config.build.run.us.running = true;
                    ...
                    //ctx.config.build.run.us.instances.push(ctx.config.build.run.instance);
                }).then(() => {
                    // update the update set
            ...
            //settings.server.hostName + (settings.server.port) ? `:${settings.server.port}` : '';

            ctx.config.application.docUri = `${settings.server.hostName}${(settings.server.port) ? `:${settings.server.port}` : ''}/doc/${ctx.config.application.id}/${ctx.config.build.sequence}`;
    ...
    CICD.prototype.buildUpdateSetOnBranch = function (options, ctx) {
        ...
        return Promise.try(() => {
            ...
            return setUpdateSet.call(this, config, 'buildUpdateSetOnBranch');
        });
    ...
    CICD.prototype.buildUpdateSet = function (options, ctx) {
        ...
        return Promise.try(() => {
            ...
            return step('install application');
            ...
            return setUpdateSet.call(this, config, 'buildUpdateSet');
        });
        ...
        return Promise.try(() => {
            ...
            return Promise.try(() => {
                ...
                return client.get({
                    ...
                    qs: {
                        'sysparm_query': 'update_set.sys_id='.concat(config.updateSet.sys_id),
            ...
            return setUpdateSet.call(this, config, 'exportUpdateSet');
        });

    };

    CICD.prototype.deployUpdateSet = function (options, ctx) {
            ...
            */
            ...
            return Promise.try(() => {
                ...
                return Promise.try(() => {
                    ...
                    return step(`Commit needs manual interaction!`);
    ...
    CICD.prototype.server = function () {
        ...
        app.route('/app/:id/us').get((req, res) => {
            ...
            return this.db.us.find({ app: req.params.id }).then((result) => {
                result = result.map((r) => {
                    if (r.host && r.host.credentials) {
                        delete r.host.credentials;
                    }
                    return r;
                });
                res.json(result);
        ...
        app.route('/app/:id/us/:us').get((req, res) => {
            console.log('req.params.id', req.params.us)
            return this.db.us.get({ _id: req.params.us }).then((result) => {
                delete result.host.credentials;
                res.json(result);
            });
        });

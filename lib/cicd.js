            ...
            */
            ...
            return Promise.each(Object.keys(applicationFilesByClass), (className) => {
                ...
                return Promise.each(chunks, (files, index) => {
                    //console.log(`\t#${index + 1}`);
                    return self.processFiles({ remote: remote, project: project }, className, files).then((filesUpdated) => {
        ...
        return self.gitPullRequestProxyConvertBody(requestBody).then((body) => {
            ...
            return Promise.try(() => {
                ...
                return Promise.try(() => {
                    ...
                    /* for now, always clean out the branch name
                    if (!merge) 
                        return;
                    
                    return Promise.try(() => {
                        return step(`remove branch name from files database`);
                    }).then(() => {
                        return self.getProject(config, config.branchName);
                    }).then((project) => {
                ...
                if (config.deploy && !config.deploy.enabled)
                    return step(`Pull request merged, but not deployment target environment specified.`);
                
                if (!config.deploy || !(config.deploy && config.deploy.onPullRequestResolve))
                    return step(`Pull request merged, but deployment 'onPullRequestResolve' is disabled!`);
                ...
                return Promise.try(() => {
                    ...
                    */
                    return new EventBusJob({ name: 'deployUpdateSet', background: true }, { commitId: config.build.commitId });
                });
            });
        });
    };

...
const CICD = (function () {
    ...
    const CICD = function (options) {
        ...
        console.log(`Version: ${chalk.red(require('../package.json').version)}`);
        ...
        // configure the 'self.settings' object
        self.setup(options);
        ...
        self.setupLightship = (forked) => {
            ...
            if (process.env.CICD_GRACEFUL_SHUTDOWN_DISABLED == 'true')
                return;

            // CICD_K8S_LIVENESS_PORT variable only valid in K8s environment
            let port = (process.env.KUBERNETES_SERVICE_HOST) ? parseInt((process.env.CICD_K8S_LIVENESS_PORT || 9000), 10) : undefined;

            // if forked, set port to undefined (random) to avoid port conflicts
            if (forked)
                port = undefined;
            ...
            const configuration = {
                ...
                port: port,
                shutdownDelay: 5 * 1000,
                shutdownHandlerTimeout: 25 * 1000,
                gracefulShutdownTimeout: parseInt(process.env.CICD_GRACEFUL_SHUTDOWN_TIMEOUT_MINUTES || 20, 10) * 60 * 1000,
                signals: self.shutdownSignals,
            };

            self.lightship = createLightship(configuration);
            console.log('[lightship] initialized');

            for (const signal of self.shutdownSignals) {
                process.on(signal, () => {
                    console.info(`[lightship] received ${signal} signal. Waiting max ${configuration.gracefulShutdownTimeout / 1000} seconds for jobs to complete before shutdown. Use 'kill -9 ${process.pid}' to force stop the process.`);
                    if (configuration.shutdownDelay) {
                        console.info('[lightship] delaying shutdown handler by %d seconds', configuration.shutdownDelay / 1000);
                    }
                });
            }

            self.on('lightship-ready', async () => {
                // put last handler last
                self.shutdownHandlers.sort((a, b) => {
                    if (a.order == 0 && b.order == 0)
                        return 0;
                    return a.order - b.order;
                });
                await Promise.all(self.shutdownHandlers.map(async ({ name, handler }) => {
                    console.log(`[lightship] Register shutdown handler for: '${name}'`);

                    self.lightship.registerShutdownHandler(async () => {
                        console.log(`[lightship] Stopping '${name}'`);
                        await handler();
                    });
                }));

                self.lightship.signalReady();
                // inform pm2 we are ready (https://pm2.keymetrics.io/docs/usage/signals-clean-restart/#graceful-start)
                if ('true' == process.env.wait_ready) {
                    process.send('ready');
                }

                const address = self.lightship.server.address();
                console.log(`[lightship] HTTP liveness service is running on port ${address.port}. Probes available under /health, /live, /ready`);
            });

            self.on('lightship-shutdown', () => {
                self.lightship.shutdown();
            });

            return self.lightship;
        };

        self.registerShutdownHandler = (name, handler, order = 10) => {
            self.shutdownHandlers.push({ name, handler, order });
        };
        self.createBeacon = (context = {}) => {
            if (self.lightship)
                return self.lightship.createBeacon(context);
            return { die: async () => undefined };
        };
        self.isServerShuttingDown = () => {
            if (self.lightship)
                return self.lightship.isServerShuttingDown();
            return false;
        };
    ...
    /**
     * Configure the self.settings object.
     * Worker nodes need to call this function after the 'register' event to have process.env changes
     * taking place in the self.settings.
     * 
     * @param {Any} options 
     */
    CICD.prototype.setup = function (options = {}) {
        const self = this;
        ...
        // dynamically load all modules - "bottom up"
        self.modules = moduleDirs.reduce((modules, dir) => {
            ...
            if (fs.existsSync(fullDir)) {
                fs.readdirSync(fullDir).forEach((file) => {
                    ...
                    if (isLibrary) {
                        if (modules[libName]) {
                            if (file != 'example-job.js') {
                                console.log(`Overloading Module: '${libName}' from Path: ${path.join(fullDir, file)}`);
                            }
                        } else {
        ...
        self.settings = assign({
            ...
            proxy: {
                ...
                strictSSL: (process.env.PROXY_STRICT_SSL == 'true')
            },
        ...
        // lookup project config templates directory - "top down"
        const buildConfigFile = self.settings.moduleDirectories.slice().reverse().reduce((fileName, dir) => {
            if (fileName)
                return fileName;

            const buildConfigJS = path.join(dir, 'project-templates', 'build-config.js');
            if (fs.existsSync(buildConfigJS)) {
                //console.log('SETUP\t', `using JavaScript build config from '${path.relative(process.cwd(), buildConfigJS)}'`);
                return buildConfigJS;
            }

            const buildConfigJson = path.join(dir, 'project-templates', 'build-config.json');
            if (fs.existsSync(buildConfigJson)) {
                //console.log('SETUP\t', `using JSON build config from '${path.relative(process.cwd(), buildConfigJson)}'`);
                return buildConfigJson;
            }

        }, null);

        const projectTemplateDir = buildConfigFile ? path.dirname(buildConfigFile) : tempDir;

        self.settings.projectTemplateDir = projectTemplateDir;
        self.settings.buildConfig = (() => {
            if (!buildConfigFile) {
                return {
                    files: [], gulp: {}
                };
            }
            if (buildConfigFile.endsWith('.js')) {
                const jsConfig = require(buildConfigFile);
                if (!jsConfig || Object.keys(jsConfig).length === 0) {
                    console.error('SETUP\t', `Build Config Error. The file '${buildConfigFile}' does not export anything. Ensure the file implements 'module.exports = defaultConfig'`);
                }
                return jsConfig;
            }
            return fs.readJSONSync(buildConfigFile);
        })();
        self.settings.moduleDirectories = undefined;
     ...
     * @returns {Promise}
     */
    CICD.prototype.init = async function (mode) {
        ...
        */
        return Promise.try(() => {
            if ('nedb' == self.settings.dataStore.type) {
                if (process.env.INSTANCE_ID && process.env.INSTANCE_ID != '0') // only the master process (INSTANCE_ID == 0) hosts the NeDB
                    return;
            ...
            */
            ...
            /*
                PM2 support to run multiple server instances
                https://pm2.keymetrics.io/docs/usage/environment/#node_app_instance-pm2-25-minimum
            */
            if (process.env.INSTANCE_ID && process.env.INSTANCE_ID != '0') // only the master process (INSTANCE_ID == 0) needs to run this
                return;
            ...
            return Promise.try(() => {
                ...
                */
                ...
                /*
                    migrate run.mergedDeployment
                */
                console.log('INIT\t', 'migrate run.mergedDeployment & run.forcedDeployment');
                await self.db.run.find({ 'config.mergedDeployment': { $exists: false } }).then((result) => {
                    return Promise.each(result, (run) => {
                ...
                /*
                    migrate run.mergedTs
                */
                console.log('INIT\t', 'migrate run.mergedTs');
                await self.db.run.find({ mergedTs: { $exists: false } }).then((result) => {
                    return Promise.each(result, (run) => {
                        run.mergedTs = run.updatedAt || run.ts;
                        return self.db.run.update(run);
                ...
                // NEDB specific migration scripts
                ...
                */
                return Promise.try(() => {
                    ...
                    return Promise.each(projectFiles, (file) => {
                        const fArr = file.split('.');
    ...
    CICD.prototype.getSlack = function () {
        ...
        return new self.Slack({
            ...
            msWebhookUri: process.env.CICD_MS_TEAMS_WEBHOOK,
     ...
     * @returns {User} 
     */
    CICD.prototype.getGitCredentials = function (config) {
        return {
     ...
     * 
     * @param {*} config the config object
     * @returns {Promise<SnProject>} 
     */
    CICD.prototype.getProject = async function (config, branch) {
        const self = this;
        if (!config)
            return null;

        await self.db.registerDataStore(config.build.applicationId);
        const dataStore = self.db[config.build.applicationId];
    ...
    CICD.prototype.setRunState = function (obj, state) {
        ...
        if (!(obj && obj.build && obj.build.runId)) {
            return self.addStep(console, obj, `ERROR: Change Run State - invalid config object '${JSON.stringify(obj)}'`);
        }
    ...
    */
    CICD.prototype.addStep = function (logger = console, config, state, error) {
        ...
        return Promise.try(() => {
            ...
            if (error) {
                ...
                logger.error(step.state, step.error);
            } else {
    ...
    CICD.prototype.email = {
        ...
        onPreviewConflicts: function ({ recipient, cc, subject, data: { name, scopeName, appName, sequence, sourceHostName, sourceUpdateSetName, sourceUpdateSetID, sourceUpdateSetUrl, targetHostName, remoteUpdateSetID, remoteUpdateSetUrl, previewProblems, dataLossWarnings } }) {

            let appDetails = [name, appName, scopeName].filter(Boolean);
            appDetails = appDetails.length ? appDetails.join(', ').concat(' : ') : '';

            subject = subject || `PREFLIGHT CONFLICTS - ${appDetails}Update Set is causing conflicts : Preview of '${sourceUpdateSetName} › #${sequence}' requires manual interaction`;
            ...
            return this.onDeploymentConflicts({ recipient, cc, subject, data: { name, scopeName, appName, sequence, sourceHostName, sourceUpdateSetName, sourceUpdateSetID, sourceUpdateSetUrl, targetHostName, remoteUpdateSetID, remoteUpdateSetUrl, previewProblems, dataLossWarnings } }, lead);

        },

        onPreviewFailure: function ({ recipient, cc, subject, data: { name, scopeName, appName, sequence, errorName, errorMessage, sourceHostName, sourceUpdateSetName, sourceUpdateSetID, sourceUpdateSetUrl, targetHostName } }) {

            let appDetails = [name, appName, scopeName].filter(Boolean);
            appDetails = appDetails.length ? appDetails.join(', ').concat(' : ') : '';

            subject = subject || `[CICD Conflict Detection Error] : ${appDetails}Preview of '${sourceUpdateSetName} › #${sequence}' failed`;
            ...
            return this.onDeploymentFailure({ recipient, cc, subject, data: { name, scopeName, appName, sequence, errorName, errorMessage, sourceHostName, sourceUpdateSetName, sourceUpdateSetID, sourceUpdateSetUrl, targetHostName } }, lead);
        },


        onDeploymentConflicts: function ({ recipient, cc, subject, data: { name, scopeName, appName, sequence, sourceHostName, sourceUpdateSetName, sourceUpdateSetID, sourceUpdateSetUrl, targetHostName, remoteUpdateSetID, remoteUpdateSetUrl, previewProblems, dataLossWarnings } }, lead) {

            let appDetails = [name, appName, scopeName].filter(Boolean);
            appDetails = appDetails.length ? appDetails.join(', ').concat(' : ') : '';

            subject = subject || `DEPLOYMENT CONFLICTS - ${appDetails}Update Set is causing conflicts: Deployment of '${sourceUpdateSetName} › #${sequence}' requires manual interaction`;
        ...
        onDeploymentHasMissingRecords: function ({ recipient, cc, subject, data: { name, scopeName, appName, sequence, sourceHostName, sourceUpdateSetName, sourceUpdateSetID, sourceUpdateSetUrl, targetHostName, remoteUpdateSetID, remoteUpdateSetUrl, missingRecords } }) {

            let appDetails = [name, appName, scopeName].filter(Boolean);
            appDetails = appDetails.length ? appDetails.join(', ').concat(' : ') : '';

            subject = subject || `DEPLOYMENT - ${appDetails}Completed with missing references! Deployment of '${sourceUpdateSetName} › #${sequence}' requires manual interaction`;
        ...
        onDeploymentFailure: function ({ recipient, cc, subject, data: { name, scopeName, appName, sequence, errorName, errorMessage, sourceHostName, sourceUpdateSetName, sourceUpdateSetID, sourceUpdateSetUrl, targetHostName } }, lead) {

            let appDetails = [name, appName, scopeName].filter(Boolean);
            appDetails = appDetails.length ? appDetails.join(', ').concat(' : ') : '';

            subject = subject || `[CICD Deployment Error] : ${appDetails}Deployment of '${sourceUpdateSetName} › #${sequence}' failed`;
    ...
    */
    CICD.prototype.setProgress = function (config, state) {
        ...
        return Promise.try(() => {
            if (!client)
                return self.addStep(console, config, `ERROR: Change Update Set status - invalid config object '${JSON.stringify(config)}'`);
     ...
     * @returns {Promise}
     */
    CICD.prototype.getApplicationTestSuites = async function (config) {
        const self = this;
        const client = self.getClient(config);
     ...
     * @param {*} config 
     */
    CICD.prototype.getApplicationTests = async function (config) {
        const self = this;
        const client = self.getClient(config);
        ...
        const files = await client.getFilesFromTable(param);
        return files.map(function (applicationTest) {
            return {
     ...
     * 
     * @param {*} config 
     * @param {Promise<Array>} testStepSysIds the testSteps sys_ids to look up
     */
    CICD.prototype.getTestsFromTestStep = async function (config, testStepSysIds) {
        const self = this;
        ...
        const filesPerChunks = await Promise.mapSeries(chunks, async (chunk, index) => {
            if (chunks.length > 1)
            ...
            const files = await client.getFilesFromTable(param);
            return files.map(function (atfStep) {
                return {
     ...
     */
    CICD.prototype.processFilesByClass = async function (config, applicationFiles = []) {
        const self = this;

        const remote = self.getClient(config.master);
        const project = await self.getProject(config, config.master.name);

        // load all files into cache
        await project.getCache();
        ...
        */
        ...
        const applicationFilesByClass = await Promise.reduce(applicationFiles, (out, file) => {
            const className = file.className || file.sys_class_name;
            if (out[className] === undefined)
                out[className] = [];

            out[className].push(file);
            return out;
        }, {});
        ...
        // callback per chunk with concurrency of 5
        await Promise.map(Object.keys(applicationFilesByClass), async (className) => {
            ...
            return Promise.map(chunks, async (files) => {
    ...
    */
    CICD.prototype.pullRequest = function (requestBody) {
        ...
        return self.gitPullRequestProxyConvertBody(requestBody).then((body) => {
            ...
            return new QueueJob({ name: 'pullRequestEvent', background: true, exclusive: true }, { body }).then((job) => {
                return { status: 'received', jobId: job._id };
            });
        });
    };

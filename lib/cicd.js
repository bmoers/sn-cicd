...
var CICD = (function () { 
    ...
    CICD.prototype.configure = function (ctx, entry) {
        ...
        return Promise.try(() => {
            ...
            ctx.branch.enabled = (get(['config', 'branch', 'name'], ctx) && get(['config', 'branch', 'host', 'name'], ctx)) ? true : false;
            
            // switch if ATF is enabled
            ctx.atf.enabled = (get(['config', 'atf', 'credentials', 'oauth', 'accessToken'], ctx)) ? true : false;

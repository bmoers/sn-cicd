    ...
    CICD.prototype.init = function (mode) {
        ...
        const buildConfigFile = self.settings.moduleDirectories.slice().reverse().reduce((fileName, dir) => {
            ...
            if (fs.existsSync(buildConfigJS)) {
                console.log('INIT\t', `using JavaScript build config from '${path.relative(process.cwd(), buildConfigJS)}'`);
            ...
            if (fs.existsSync(buildConfigJson)) {
                console.log('INIT\t', `using JSON build config from '${path.relative(process.cwd(), buildConfigJson)}'`);
        ...
        self.settings.buildConfig = (() => {
            ...
            if (buildConfigFile.endsWith('.js')) {
                ...
                if (!jsConfig || Object.keys(jsConfig).length === 0) {
                    console.error('INIT\t', `Build Config Error. The file '${buildConfigFile}' does not export anything. Ensure the file implements 'module.exports = defaultConfig'`);
                }
                return jsConfig;
            }
